...
export async function findMatch(userId, difficulty, topics) {
  for (const topic of topics) {
    ...
    if (waitingUserId && waitingUserId !== userId) {
      ...
      await removeUserFromAllQueues(waitingUserId, matchedUserData.difficulty, parsedTopics);

      const matchId = await createPendingMatch(userId, waitingUserId, difficulty, parsedTopics, topic);
  ...
  await redis.hmset(getUserKey(userId), { difficulty, topics: JSON.stringify(topics) });
